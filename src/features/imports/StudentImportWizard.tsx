/**
 * 7-Stage Student Import Wizard (Phase B).
 *
 * Stage flow:
 *   1. Upload     — select the institutional .xlsx workbook
 *   2. System check — file/sheet/header detection and column mapping
 *   3. Data quality  — per-row parsing verdicts (DOB/status/admission/gender)
 *   4. DB cross-check — plan partition vs existing records (new/update/unchanged)
 *   5. Review      — full read-only per-row breakdown
 *   6. Confirm     — all-or-nothing atomic student write
...
type RowFilter = 'all' | ApiStudentImportPlan;
...
export const StudentImportWizard: React.FC<StudentImportWizardProps> = ({
  ...
  const [preview, setPreview] = useState<ApiStudentImportPreview | null>(null);
  const [confirmResult, setConfirmResult] = useState<ApiStudentImportConfirm | null>(null);
  ...
  const handleAnalyze = async () => {
    ...
    setIsUploading(true);
    try {
      const result = await studentImportService.preview(selectedFile);
      setPreview(result);
  ...
  const handleConfirm = async () => {
    ...
    try {
      const result = await studentImportService.confirm(preview.session_uuid);
      setConfirmResult(result);
      setStage(7);
      showToast({
        title: 'Student import complete',
        message: `Committed ${result.result.created} new, ${result.result.updated} updated student records${
  ...
  const planCounts = preview ? countStudentPlans(preview.rows) : null;
  const partitions = preview ? partitionStudentRows(preview.rows) : null;
  const blocked = summary ? isStudentImportBlocked(summary) : false;

  const visibleRows = (): ApiStudentImportRow[] => {
    if (!preview) return [];
    if (rowFilter === 'all') return preview.rows;
    return preview.rows.filter(r => r.plan === rowFilter);
  };

  const planBadge = (plan: ApiStudentImportPlan) => (
    <Badge variant={STUDENT_PLAN_BADGE_VARIANT[plan]}>{STUDENT_PLAN_LABELS[plan]}</Badge>
  );
  ...
  // All preview-dependent stages share the same summary fac card set; only the
  // emphasized content differs per stage.
  const summaryCards = () => {
    if (!summary || !planCounts) return null;
    return (
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <div className="bg-white p-3 rounded-xl border border-slate-200">
          <p className="text-[11px] text-slate-500 uppercase font-semibold">New students</p>
          ...
          {stage === 3 && preview && summary && (
            <div className="space-y-5">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Per-row parsing verdicts (DOB, status, admission year, gender, blank placements).
                </p>
              </div>
              {summaryCards()}
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <div className="p-4 bg-white rounded-xl border border-slate-200">
                <p className="text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Placement resolution</p>
                <p className="text-sm text-slate-600">
                  {summary.placement.changed} will change placement • {summary.placement.blank} blank •
                  {summary.placement.unresolved} unresolved
                </p>
                {summary.placement.program_tokens.length > 0 && (
                  <p className="text-xs text-slate-500 mt-1">
                    Program tokens detected (informational): {summary.placement.program_tokens.join(', ')}
                  </p>
                )}
              </div>
            </div>
          ...
          {stage === 4 && preview && summary && (
            <div className="space-y-5">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Match result against existing student records (by student ID; email is conflict-checked).
                </p>
              </div>
              {summaryCards()}
              ...
              <div className="p-4 bg-white rounded-xl border border-slate-200">
                ...
                <p className="text-sm text-slate-600">{accountPlanSummary(summary.accounts)}</p>
              ...
              {!blocked && (
                <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-200 flex items-start gap-3">
                  ...
                  <div className="text-xs text-emerald-800">
                    ...
                    <p>No blocking errors — the all-or-nothing confirm can proceed greedily.</p>
                  </div>
                </div>
              )}
            </div>
                ...
                <div className="overflow-x-auto max-h-[30rem]">
                  <table className="w-full text-xs text-left">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {visibleRows().map(row => {
                        const display = studentRowDisplay(row);
                        const account = accountActionForRow(row.account);
                        ...
                        return (
                          <tr key={row.row} className="hover:bg-slate-50 align-top">
                            ...
                            <td className="px-3 py-2">
                              ...
                              <p className="text-[11px] text-slate-500">{display.studentId}</p>
                              <p className="text-[11px] text-slate-400">{display.email}</p>
                            </td>
                            <td className="px-3 py-2 text-slate-700">
                              <p>{placementSummary(row.placement)}</p>
                              <p className="text-[11px] text-slate-400">Roll {display.rollNo}</p>
                            </td>
                            ...
                            <td className="px-3 py-2 space-y-1">
                              ...
                              {shownChanges.length > 0 && (
                                <p className="text-[11px] text-emerald-700 bg-emerald-50 rounded px-1.5 py-1">
                                  <Filter className="w-3 h-3 inline mr-1" />
                                  {fieldChangeSummary(shownChanges)}
                                </p>
                              )}
                            </td>
                          </tr>
            ...
            <div className="space-y-6">
              {blocked ? (
                ...
                <div className="max-w-xl mx-auto py-6 text-center space-y-4">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-slate-900">Ready to Ingest Students</h3>
                    <p className="text-xs text-slate-600 mt-1">
                      You are about to commit {planCounts?.new ?? 0} new students and {planCounts?.update ?? 0} updates.
                  ...
                  <div className="p-4 bg-slate-900 text-slate-200 rounded-xl text-left text-xs space-y-2">
                    ...
                    <p className="text-slate-400">
                      ...
                      If any row has an error or conflict the entire import is refused and nothing is written.
                    </p>
                  </div>

                  {isConfirming && (
                    <div className="space-y-2 pt-4">
                      <p className="text-xs text-slate-500 font-mono">Committing student records…</p>
                    </div>
                  )}
                </div>
              )}
