/**
 * Teacher import history modal (Phase C).
 *
 * Lists the importing admin's own teacher import sessions (pending/confirmed)
 * with the persisted plan summary and, when confirmed, the
...
interface TeacherImportHistoryModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export const TeacherImportHistoryModal: React.FC<TeacherImportHistoryModalProps> = ({
  isOpen,
  onClose,
}) => {
  ...
  useEffect(() => {
    if (isOpen) {
      teacherImportService.getHistory().then(setHistory).catch(() => setHistory([]));
    }
  }, [isOpen]);

  if (!isOpen) return null;

  /** Backend summary for teacher sessions always carries plans + changes. */
  ...
  const changes = (session: ApiImportSession): { rows_with_changes: number; change_fields: string[] } | null => {
    const c = (session.summary as { changes?: { rows_with_changes: number; change_fields: string[] } }).changes;
    return c || null;
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="bg-slate-900 text-white px-6 py-4 flex items-center justify-between border-b border-slate-800">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white">Teacher Import History</h3>
        ...
        <div className="p-6 overflow-y-auto flex-1 bg-slate-50/50 space-y-4">
          {history.length === 0 ? (
            <div className="text-center py-12 text-slate-500 text-sm">No teacher import sessions exist yet.</div>
          ) : (
            <div className="space-y-3">
              {history.map(session => {
                const sessionPlans = plans(session);
                const confirmTotals = confirmed(session);
                const sessionChanges = changes(session);
                      ...
                      <div className="mt-4 pt-4 border-t border-slate-100 text-xs bg-slate-50/70 p-3 rounded-lg animate-in fade-in space-y-3">
                        ...
                        {sessionChanges && (
                          <div className="grid grid-cols-2 gap-3 border-t border-slate-200 pt-3">
                            <div>
                              <span className="text-slate-500 font-medium">Rows with changes:</span>
                              <p className="font-bold text-slate-800">{sessionChanges.rows_with_changes}</p>
                            </div>
                            <div>
                              <span className="text-slate-500 font-medium">Changed fields:</span>
                              <p className="font-bold text-slate-800">
                                {sessionChanges.change_fields.length
                                  ? sessionChanges.change_fields.map(f => f.replace(/_/g, ' ')).join(', ')
                                  : 'None'}
                              </p>
                            </div>
                          </div>
